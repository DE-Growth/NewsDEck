...
class IntegratedNewsProducer:
    ...
    def _create_kafka_producer(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Kafka 연결 실패 (시도 {attempt + 1}/{max_retries}). {retry_delay}초 후 재시도..."
                )
    ...
    def search_news(self, query, display=None):
        ...
        params = {"query": query, "display": display, "sort": "date", "start": 1}
    ...
    def send_to_kafka(self, data):
        ...
        try:
            ...
            logger.debug(
                f"Kafka 전송 성공: {record_metadata.topic}[{record_metadata.partition}] offset {record_metadata.offset}"
            )
            return True
    ...
    def process_article(self, item, keyword):
        ...
        try:
            ...
            if not title or not item.get("link"):
                logger.warning(
                    f"필수 데이터 누락: title={bool(title)}, link={bool(item.get('link'))}"
                )
                return None
        ...
        try:
            ...
            for article in articles:
                ...
                if self.send_to_kafka(news_data):
                    ...
                    logger.error(
                        f"[{keyword}] '{news_data['title'][:50]}...' 전송 실패"
                    )
            ...
            logger.info(
                f"[{keyword}] 수집 완료: {success_count}/{len(articles)}건 성공"
            )
    ...
    def run_once(self):
        ...
        logger.info(
            f"=== 수집 사이클 완료: 총 {total_collected}건 수집 (소요시간: {elapsed_time:.1f}초) ==="
        )
